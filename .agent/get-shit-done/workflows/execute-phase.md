...
5.6. **Post-wave shared artifact update (worktree mode only):**
   ...
   for PLAN_ID in ${WAVE_PLAN_IDS}; do
     node ".agent/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}" "${PLAN_ID}" completed
   done
...
```bash
SCHEMA_DRIFT=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
